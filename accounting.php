...
/**
 * Функция расчёта пени за просрочку оплаты
 */
function calculatePenaltyCurrent($debtAmount, $paymentDeadline)
{
    if ($debtAmount <= 0)
        return 0;

    $deadline = new DateTime($paymentDeadline);
    $today = new DateTime();
    ...
    $calculatedPenalty = $debtAmount * $dailyPenaltyRate * $daysOverdue;
    $maxPenalty = $debtAmount * 0.1;

    return min($calculatedPenalty, $maxPenalty);
}

// Обработка сдвига дат НАЗАД на 10 дней (для тестирования просрочек)
if (isset($_POST['action']) && $_POST['action'] === 'shift_dates') {
    header('Content-Type: application/json');
    try {
        $pdo->exec("UPDATE contracts SET payment_deadline = DATE_SUB(payment_deadline, INTERVAL 10 DAY)");
        echo json_encode(['success' => true, 'message' => 'Дата сдвинута на 10 дней']);
    } catch (Exception $e) {
...
if (isset($_POST['action']) && $_POST['action'] === 'repay_debt') {
    header('Content-Type: application/json');
    try {
        ...
        $stmt = $pdo->prepare("
            SELECT c.total_amount, COALESCE(SUM(ao.amount), 0) as paid_amount
            FROM contracts c
            LEFT JOIN accounting_operations ao ON c.id = ao.contract_id AND ao.operation_type = 'оплата'
            WHERE c.id = ?
            GROUP BY c.id
        ");
        ...
        $current_debt = $contract_data['total_amount'] - $contract_data['paid_amount'];

        if ($repay_amount <= 0 || $repay_amount > $current_debt) {
            echo json_encode(['success' => false, 'message' => 'Некорректная сумма погашения']);
            exit;
        }

        $stmt = $pdo->prepare("
            INSERT INTO accounting_operations (contract_id, operation_date, operation_type, amount, payment_type, document_number, comment)
            VALUES (?, CURDATE(), 'оплата', ?, 'безналичные', CONCAT('Погашение долга-', DATE_FORMAT(NOW(), '%Y%m%d%H%i%s')), 'Погашение дебиторской задолженности')
        ");
        $stmt->execute([$contract_id, $repay_amount]);

        echo json_encode(['success' => true, 'message' => 'Долг погашен на сумму ' . number_format($repay_amount, 2, ',', ' ') . ' ₽']);
    } catch (Exception $e) {
        echo json_encode(['success' => false, 'message' => $e->getMessage()]);
    }
    exit;
...
// Обработка списания пени
if (isset($_POST['action']) && $_POST['action'] === 'write_off_penalty') {
    header('Content-Type: application/json');
    try {
        $contract_id = $_POST['contract_id'];
        ...
        $stmt = $pdo->prepare("
            INSERT INTO accounting_operations (contract_id, operation_date, operation_type, amount, payment_type, document_number, comment)
            VALUES (?, CURDATE(), 'пеня', ?, 'безналичные', CONCAT('Пеня-', DATE_FORMAT(NOW(), '%Y%m%d%H%i%s')), 'Начисление пени за просрочку оплаты')
        ");
        $stmt->execute([$contract_id, $penalty_amount]);

        echo json_encode(['success' => true, 'message' => 'Пеня на сумму ' . number_format($penalty_amount, 2, ',', ' ') . ' ₽ начислена']);
    } catch (Exception $e) {
        echo json_encode(['success' => false, 'message' => $e->getMessage()]);
    }
    exit;
...
// Получаем дебиторскую задолженность (актуальную на сегодня) с расчётом пени
$debt_data = $pdo->query("
    SELECT 
        c.id as contract_id,
        c.contract_number,
    ...
    WHERE c.status IN ('подписан', 'утвержден', 'отгружен', 'в боксе')
...
// Разделяем на текущую и просроченную задолженность с расчётом пени
$current_debts = [];
$overdue_debts = [];
$total_current_debt = 0;
$total_overdue_debt = 0;
$total_overdue_debt_with_penalty = 0;
$total_penalty = 0;

foreach ($debt_data as &$debt) {
    $principal_debt = $debt['debt_amount'];
    $current_penalty = calculatePenaltyCurrent($principal_debt, $debt['payment_deadline']);
    $debt['current_penalty'] = $current_penalty;
    $debt['total_with_penalty'] = $principal_debt + $current_penalty;
    ...
    $days_until_overdue = $today <= $deadline ? $today->diff($deadline)->days : 0;
    $debt['days_overdue'] = $days_overdue;
    $debt['days_until_overdue'] = $days_until_overdue;

    if ($days_overdue > 0) {
        $overdue_debts[] = $debt;
        $total_overdue_debt += $principal_debt;
        $total_overdue_debt_with_penalty += $principal_debt + $current_penalty;
    } else {
        $current_debts[] = $debt;
        $total_current_debt += $principal_debt;
    }
}
unset($debt);
...
<head>
    ...
    <style>
        ...
        /* Стили для панели задолженности */
        .debt-panel {
            background: white;
            border-radius: 15px;
            padding: 0;
        ...
        .debt-summary-item:first-child {
            border-right: 1px solid #e9ecef;
        }

        .debt-summary-item.active {
            background-color: #f8f9fa;
        }

        .debt-summary-item:hover {
            background-color: #f8f9fa;
        }
        ...
        .debt-summary-amount.current {
            color: #f39c12;
        }

        .debt-summary-amount.overdue {
            color: #e74c3c;
        }
...
<body>
    ...
    <main class="container-fluid px-4">
        ...
        <div class="row mb-4">
            ...
            <div class="col-md-4 mb-3">
                <div class="kpi-card">
                    ...
                    <small class="text-muted">На основе
                        <?= $feedback_analysis['total_feedbacks'] ?? 0 ?> оценок
                    </small>
                </div>
            </div>
        </div>
        ...
        <!-- Кнопка сдвига дат -->
        <div class="shift-button-wrapper">
            <button class="btn test-btn" id="shiftDatesBtn">
                <i class="bi bi-calendar-minus me-1"></i>Сдвинуть дату на 10 дней
            </button>
        </div>

        <!-- Панель дебиторской задолженности (обновленная) -->
        <div class="debt-panel">
            <div class="debt-header d-flex justify-content-between align-items-center">
                <h4><i class="bi bi-currency-ruble me-2"></i>Дебиторская задолженность</h4>
                <small class="text-white-50"></small>
            </div>
            <div class="debt-summary">
                <div class="debt-summary-item" data-tab="current">
                    <div class="debt-summary-label">Текущая задолженность</div>
                    <div class="debt-summary-amount current"><?= formatAmount($total_current_debt) ?></div>
                </div>
                <div class="debt-summary-item" data-tab="overdue">
                    <div class="debt-summary-label">Просроченная задолженность</div>
                    <div class="debt-summary-amount overdue"><?= formatAmount($total_overdue_debt_with_penalty) ?></div>
                </div>
            </div>

            <div id="currentDebtTable" class="debt-table">
                <?php if (empty($current_debts)): ?>
                    <p class="text-center text-muted my-3">Нет текущей задолженности</p>
                <?php else: ?>
                        ...
                        <table class="table table-hover">
                            <thead class="table-light">
                                <tr>
                                    ...
                                    <th>Оплачено</th>
                                    <th>Задолженность</th>
                                    <th>Дней до просрочки</th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php foreach ($current_debts as $debt): ?>
                                    <tr>
                                        <td><?= htmlspecialchars($debt['contract_number']) ?></td>
                                        <td><?= htmlspecialchars($debt['customer_name']) ?></td>
                                        <td><?= date('d.m.Y', strtotime($debt['valid_from'])) ?></td>
                                        <td><?= date('d.m.Y', strtotime($debt['payment_deadline'])) ?></td>
                                        <td><?= formatAmount($debt['total_amount']) ?></td>
                                        <td><?= formatAmount($debt['paid_amount']) ?></td>
                                        <td class="text-danger fw-bold"><?= formatAmount($debt['debt_amount']) ?></td>
                                        <td><span class="debt-badge current">осталось <?= $debt['days_until_overdue'] ?>
                                                дн.</span></td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
            ...
            <div id="overdueDebtTable" class="debt-table" style="display: none;">
                <?php if (empty($overdue_debts)): ?>
                    <p class="text-center text-muted my-3">Нет просроченной задолженности</p>
                <?php else: ?>
                    <div class="table-responsive">
                        <table class="table table-hover">
                            <thead class="table-light">
                            ...
                            <tbody>
                                <?php foreach ($overdue_debts as $debt): ?>
                                    <tr>
                                        <td><?= htmlspecialchars($debt['contract_number']) ?></td>
                                        <td><?= htmlspecialchars($debt['customer_name']) ?></td>
                                        <td><?= date('d.m.Y', strtotime($debt['payment_deadline'])) ?></td>
                                        ...
                                        <td><?= formatAmount($debt['paid_amount']) ?></td>
                                        <td class="text-danger fw-bold"><?= formatAmount($debt['debt_amount']) ?></td>
                                        <td class="penalty-cell"><span
                                                class="penalty-amount"><?= formatAmount($debt['current_penalty']) ?></span>
                                        </td>
                                        <td class="text-danger fw-bold"><?= formatAmount($debt['total_with_penalty']) ?>
                                        </td>
                                        <td><span class="debt-badge overdue">+<?= $debt['days_overdue'] ?> дн.</span>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
                        ...
                        <?php foreach ($operations as $op): ?>
                            <tr>
                                ...
                                <td>
                                    ...
                                    <?php elseif ($op['operation_type'] == 'пеня'): ?>
                                        <span class="badge bg-danger">Пеня</span>
                                    <?php else: ?>
                                ...
                                <td class="fw-bold <?= $op['operation_type'] == 'пеня' ? 'text-danger' : '' ?>">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header bg-success text-white">
                    <h5 class="modal-title"><i class="bi bi-credit-card me-2"></i>Погашение задолженности</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <p>Договор: <strong id="repayContractNumber"></strong></p>
                    <p>Текущая задолженность: <strong id="repayCurrentDebt" class="text-danger"></strong></p>
                    <div class="mb-3">
                        <label class="form-label">Сумма погашения</label>
                        <input type="number" step="0.01" class="form-control" id="repayAmount"
                            placeholder="Введите сумму">
                        <button class="btn btn-outline-success btn-sm mt-2" id="repayFullBtn">Погасить
                            полностью</button>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Отмена</button>
                    <button type="button" class="btn btn-success" id="confirmRepayBtn">Погасить</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        // Переключение между текущей и просроченной задолженностью
        let currentTab = 'current';
        $('.debt-summary-item').click(function () {
            const tab = $(this).data('tab');
            currentTab = tab;
            $('.debt-summary-item').removeClass('active');
            $(this).addClass('active');
            if (tab === 'current') {
                $('#currentDebtTable').show();
                $('#overdueDebtTable').hide();
            } else {
                $('#currentDebtTable').hide();
                $('#overdueDebtTable').show();
            }
        });
        $('.debt-summary-item:first').addClass('active');
        ...
        // Погашение долга
        let currentRepayContractId = null, currentRepayDebt = 0;
        $('.repay-btn').click(function () {
            currentRepayContractId = $(this).data('id');
            currentRepayDebt = parseFloat($(this).data('debt'));
            $('#repayContractNumber').text($(this).data('number'));
            $('#repayCurrentDebt').text(currentRepayDebt.toLocaleString() + ' ₽');
            $('#repayAmount').val('');
            $('#repayModal').modal('show');
        });
        $('#repayFullBtn').click(function () { $('#repayAmount').val(currentRepayDebt); });
        $('#confirmRepayBtn').click(function () {
            ...
            if (repayAmount > currentRepayDebt) {
                Swal.fire('Ошибка', 'Сумма погашения не может превышать задолженность', 'error');
                return;
            }
            $.ajax({
                url: 'accounting.php',
                method: 'POST',
                data: { action: 'repay_debt', contract_id: currentRepayContractId, repay_amount: repayAmount },
                dataType: 'json',
                success: function (response) {
                    if (response.success) {
                        Swal.fire('Успешно', response.message, 'success').then(() => location.reload());
                    } else {
        ...
        // Начисление пени
        $('.write-off-penalty-btn').click(function () {
            const contractId = $(this).data('id'), contractNumber = $(this).data('number'), penaltyAmount = parseFloat($(this).data('penalty'));
            Swal.fire({
                title: 'Начисление пени',
                html: `Договор: <strong>${contractNumber}</strong><br>Сумма пени: <strong class="text-danger">${penaltyAmount.toLocaleString()} ₽</strong><br><br><small class="text-muted">Расчёт: 0.1% от суммы долга за каждый день просрочки, но не более 10% от суммы долга.</small>`,
                icon: 'warning',
                showCancelButton: true,
                confirmButtonText: 'Да, начислить',
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: 'accounting.php',
                        method: 'POST',
                        data: { action: 'write_off_penalty', contract_id: contractId, penalty_amount: penaltyAmount },
                        dataType: 'json',
                        success: function (response) {
                            if (response.success) {
                                Swal.fire('Успешно', response.message, 'success').then(() => location.reload());
        ...
        // Сдвиг дат
        $('#shiftDatesBtn').click(function () {
            Swal.fire({
                title: 'Сдвиг даты',
                text: 'Вы уверены, что хотите сдвинуть все даты на 10 дней?',
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#8b5cf6',
                confirmButtonText: 'Да, сдвинуть',
                cancelButtonText: 'Отмена'
            }).then((result) => {
                if (result.isConfirmed) {
                    $.ajax({
                ...
                <div style="font-family: 'Times New Roman', serif;">
                    ...
                    <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px; border: 1px solid #000;">
                        ...
                        <tr style="border: 1px solid #000; background-color: #f5f5f5;">
                            ...
                            <td style="padding: 8px; border: 1px solid #000;" colspan="3"><strong style="font-size: 1.2rem;">${formattedAmount}</strong></td>
                        </tr>
                        <tr style="border: 1px solid #000;">
